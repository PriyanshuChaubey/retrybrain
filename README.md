...
- **What's simulated:** the recovery *outcomes* come from `simulator.py`, not a live payment gateway — so the results demonstrate the architecture, policy, and measurement, not production revenue. The oracle is never seen by the model or the allocator, and the baseline is scored against that same oracle, so the comparison is like-for-like. Swapping the simulator for a real PSP is a single isolated change.
- **Why 88% and not 100%:** the seven unrecovered payments are shown on the dashboard, not hidden — four expired-card declines where the customer never completes the alternate-method flow (one on DND, so no contact is allowed), one insufficient-funds payment that hit the max-attempts cap, and two `other` declines. Each is handed to a human rather than silently dropped.
- **The economics priors are stated assumptions, not fitted numbers:** the per-action costs and per-cause conversion rates in `economics.py` are values a finance team would negotiate, kept deliberately separate from the oracle — so the allocator is tested against domain priors, never handed the answer.
- **Main tunable levers:** `RETRY_THRESHOLD` (the recovery-vs-cost trade-off), `MAX_ATTEMPTS`, and the per-cause timing rules in `optimal_retry`.
